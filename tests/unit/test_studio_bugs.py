...
def test_rebuild_graph_references_sigma_container():
    ...
    m = re.search(r"async function rebuildGraph\(\)(.*?)(?=\nasync function|\nfunction )", content, re.DOTALL)
...
def test_layout_reads_edge_kind_attribute():
    ...
    content = _html()
    layout_match = re.search(r"function applyLayout\((.*?)(?=\nfunction )", content, re.DOTALL)
...
def test_layout_uses_stronger_force_for_scope_edges():
    ...
    has_scope_branch = bool(
        re.search(r"scope", body)
        and re.search(r"scope.*\d|\d.*scope", body)
    )
...
def test_init_graph_uses_sigma_container():
    ...
    content = _html()
    m = re.search(r"async function initGraph\(\)(.*?)(?=\nasync function|\nfunction )", content, re.DOTALL)
